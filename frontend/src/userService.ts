...
export async function updateMyStatus(status: UserStatus): Promise<void> {
  try {
    await fetchApi<ApiResponse<null>>('/api/users/me/status', 'PUT', {
      status,
    });
...
export function updateMyStatusOnUnload(): void {
  const token = localStorage.getItem('authToken');
  if (!token) {
    return;
  }
  ...
  try {
    fetch(url, {
      ...
      console.error('Failed to send offline status on unload:', error);
    });
    console.log('Offline status update fetch with keepalive initiated.');
  } catch (error) {
    console.error('Error initiating offline status update fetch:', error);
  }
}
